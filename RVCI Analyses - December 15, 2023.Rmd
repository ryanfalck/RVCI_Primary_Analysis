...
PRIMARY OUTCOME: ADAS-Cog Plus
```{r message=FALSE}
lmm.ex.mdl("ADASCOGPlusDSST", "ADASCOGPlusDSSTbaseline", baseline.sex.adasplusDSST.mean, baseline.sex.adasplusDSST.se)
```
...
SECONDARY OUTCOME: ADAS-Cog 13
```{r message=FALSE}
lmm.ex2.mdl("ADASCogTotal", "ADASCogTotalbaseline", baseline.sex.adas11.mean, baseline.sex.adas11.se)
```
...
lmm.ex3.mdl<- function(y,cov1,baselinemean, baselinese){
  ...
  return(list(summary=modelsummary, anova_results = modelanova, lsmeans=means.1, group_contrasts=contrasts, contrasts_ci.95=conf.95, post_hoc=posthoc))
}
```

SECONDARY OUTCOME: Gait Speed
```{r}
lmm.ex3.mdl("GaitSpeed", "GaitSpeedbaseline", baseline.sex.Gait.mean, baseline.sex.Gait.se)
```

SECONDARY OUTCOME: SPPB
```{r}
lmm.ex3.mdl("SPPB", "SPPBbaseline", baseline.sex.SPPB.mean, baseline.sex.SPPB.se)
```

SECONDARY OUTCOME: PPA
```{r message=FALSE}
lmm.ex3.mdl("PPAScore", "PPAScorebaseline", baseline.sex.PPA.mean, baseline.sex.PPA.se)
```

SECONDARY OUTCOME: Quad Strength
```{r message=FALSE}
lmm.ex3.mdl("MaxQuadStrength", "MaxQuadStrengthbaseline", baseline.sex.Quad.mean, baseline.sex.Quad.se)
```

SECONDARY OUTCOME: PASE
```{r message=FALSE}
lmm.ex3.mdl("PASE", "PASEbaseline", baseline.sex.PASE.mean, baseline.sex.PASE.se)
```

SECONDARY OUTCOME: Pulse Wave Velocity
```{r message=FALSE}
lmm.ex3.mdl("CFPWV", "CFPWVbaseline", baseline.sex.PWV.mean, baseline.sex.PWV.se)
```
