...
RVCI <- read_excel("REVISED RVCI MASTER - April 8, 2024.xlsx")
...
Tableone.df2<-subset(Tableone.df, ID != "RVCI_035" & ID != "RVCI_084" & ID != "RVCI_086" & ID != "RVCI_088")


vars<-dput(names(Tableone.df2[c(6,7,10,11,93,94,107,17,21:24,78,79,
                                29:31,34:37,77,81,82,95:102,46:57,83,
...
wide.data3<-wide.data2[c(1:74,167:172,75:166)]


data6 <- reshape(as.data.frame(wide.data3),idvar="ID",varying=81:172,direction="long",sep=".") #reshape to long data frame with T2 and T3 repeated outcome and baseline as separate time-invariant
...
lmm.ex.mdl<- function(y,cov1,baselinemean, baselinese){
  model <- lmer(as.formula(paste0(y, "~factor(Group)*factor(time) +", cov1, " + MoCAbaseline + Sex + EDU  + FraminghamRiskScoreBloodPanel + PASEbaseline + Medications + SupinebPPbaseline + FazekasScore + COVIDInfection + COVIDAffected + ADASCOGPlusDSSTsebaseline + (1|ID)")), final.exclude)
...
lmm.ex3.mdl<- function(y,cov1,baselinemean, baselinese){
  model <- lmer(as.formula(paste0(y, "~factor(Group)*factor(time) +", cov1, " + MoCAbaseline + Sex + EDU  + FraminghamRiskScoreBloodPanel + Medications + SupinebPPbaseline + FazekasScore + COVIDInfection + COVIDAffected + (1|ID)")), final.exclude)
  modelsummary <- summary(model)
  means.1 <- lsmeans(model, ~Group|time)
  contrasts <- contrast(means.1, "trt.vs.ctrl", adj="none")
  conf.95<-confint(contrasts, parm, level = 0.95)
...
```{r message=FALSE}
lmm.ex.mdl<- function(y,cov1,baselinemean, baselinese){
  model <- lmer(as.formula(paste0(y, "~factor(Group)*factor(time)*Sex +", cov1, "+ MoCAbaseline + EDU  + FraminghamRiskScoreBloodPanel + PASEbaseline + Medications + SupinebPPbaseline + FazekasScore + COVIDInfection + COVIDAffected + ADASCOGPlusDSSTsebaseline +
...
```{r message=FALSE}
lmm.ex3.mdl<- function(y,cov1,baselinemean, baselinese){
  model <- lmer(as.formula(paste0(y, "~factor(Group)*factor(time)*Sex +", cov1, "+ MoCAbaseline + EDU  + FraminghamRiskScoreBloodPanel + Medications + SupinebPPbaseline + FazekasScore + COVIDInfection + COVIDAffected +
                                  (1|ID)")), data= final)
  modelsummary <- summary(model)
  modelanova <- anova(model)
  means.1 <- lsmeans(model, ~Group|time|Sex)
